...
mod tools {
  ...
  impl<'a> Patchwork<'a> for Tester {
    ...
    fn key_type() -> protean::KeyType {
      protean::KeyType::Global
    }
  }
  ...
  impl<'a> Patchwork<'a> for Nested {
    ...
    fn key_type() -> protean::KeyType {
      protean::KeyType::Local
    }

    fn get_key(&self) -> Result<uuid::Uuid> {
      unimplemented!("'Replicant::Nested::get_key' still needs to be implemented")
    }
  }
  ...
  impl<'a> Patchwork<'a> for Unkeyed {
    ...
    fn get_key(&self) -> Result<uuid::Uuid> {
      unimplemented!("'Replicant::Unkeyed::get_key' still needs to be implemented")
    }
  }
}
...
test!(
  fn test_diff() {
    ...
    log::debug!("Self Test:\n{:#?}", patch);
    assert!(patch.unwrap().is_empty(), "Patch was not empty");
...
test!(
  fn test_replicant_register() {
    ...
    let result = register!(primary, tools::Tester, opts).unwrap();
    log::debug!("result:\n{:#?}", result);

    // Assert there is a Creche for Tester.
    let stats = primary.get_stats();
    assert_eq!(1, stats.len());

    // Assert that there is an empty creche for Tester
